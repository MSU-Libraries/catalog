...
/**
 * LocationNotices helper factory.
 *
 * PHP version 8
 *
 ...
 * @link     https://vufind.org/wiki/development Wiki
 */
class LocationNoticesFactory implements FactoryInterface
{
    /**
     * Create an object
     *
    ...
    public function __invoke(
        ...
        try {
            $noticesConfig = $container->get(\VuFind\Config\YamlReader::class)
                ->get('LocationNotices.yaml');
        } catch (\Exception $e) {
            $logger = $container->get(\VuFind\Log\Logger::class);
            $logger->err(
                'Could not parse LocationNotices.yaml: ' . $e->getMessage()
            );
        }
