...
function toggleExpansion(item) {
  ...
  item = item.target;
  let leveledCheckbox = $(item).closest('.leveledCheckbox');
...
function toggleCheck(item) {
  ...
  item = $(item.target).parents('.leveledCheckbox');
  if (!item.hasClass('leveledCheckbox')) return;

  let itemChecked = $(item).find('input[type="checkbox"]').prop('checked');

  // Modifying children
  // If the select checkbox contains a sub selection, check / uncheck the sub selection
  let nextItem = toggleChildren(itemChecked, item);
  ...
  checkAndUpdatePreviousLeveledCheckboxes(item, indeterminate, false);
}
...
function checkAndUpdatePreviousLeveledCheckboxes(itemToStartWith, indeterminate = undefined, runAllTheList = false) {
  let tmp, currentElementChecked, currentElementIndeterminate;
  ...
  while (previousItem.length > 0 && (currentLevel > 0 || runAllTheList)) {
    ...
    if (tmp < currentLevel) {
      ...
      currentElementChecked = $(previousItem).find('input[type="checkbox"]').prop('checked');
      currentElementIndeterminate = $(previousItem).find('input[type="checkbox"]').prop('indeterminate');
      if (itemChecked !== currentElementChecked || currentElementIndeterminate) {
        indeterminate = true;
      }
    }
