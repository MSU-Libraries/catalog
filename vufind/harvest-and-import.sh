...
batch_import() {
    ...
    # No countdown here to avoid slowing down the Solr cache warmup after the commit that happened in the previous step
    ...
    verbose "Starting Solr cache warmup"
    for n in 1 2 3; do
        # This is the largest of the 2 requests the catalog will do to Solr for a search from the home page without a search string.
        # It might need to be updated with future VuFind versions; the data can be obtained from the Solr log.
        curl --silent --show-error "http://solr$n:8983/solr/biblio/select" --data 'mm=0%25&facet.field={!ex%3Dinstitution_filter}institution&facet.field={!ex%3Dbuilding_filter}building&facet.field={!ex%3Dmaterial-type_str_mv_filter}material-type_str_mv&facet.field={!ex%3Dformat_filter}format&facet.field={!ex%3Dauthor_facet_filter}author_facet&facet.field={!ex%3DpublishDate_filter}publishDate&facet.field={!ex%3Dgenre_facet_filter}genre_facet&facet.field={!ex%3Dpublisher_txtF_mv_filter}publisher_txtF_mv&facet.field={!ex%3Dtopic_facet_filter}topic_facet&facet.field={!ex%3Dlanguage_filter}language&facet.field={!ex%3Dcallnumber-first_filter}callnumber-first&facet.field={!ex%3Dgeographic_facet_filter}geographic_facet&facet.field={!ex%3Dera_facet_filter}era_facet&spellcheck.dictionary=default&qt=edismax&hl=true&json.nl=arrarr&fl=*&start=0&fq={!tag%3Dinstitution_filter}institution:(institution:"Michigan+State+University")&fq=suppress-from-discovery_boolean:"false"&sort=title_sort+asc,id+asc&rows=20&hl.simple.pre={{{{START_HILITE}}}}&facet.limit=30&q=*:*&spellcheck.q=&hl.simple.post={{{{END_HILITE}}}}&spellcheck=true&qf=title_short^750+title_full_unstemmed^600+title_full^400+title^500+title_alt^200+title_new^100+series^50+series2^30+author^300+contents^10+topic_unstemmed^550+topic^500+geographic^300+genre^300+allfields_unstemmed^10+fulltext_unstemmed^10+allfields+fulltext+description+isbn+issn+long_lat_display&facet.mincount=1&hl.fl=title_short,title_full_unstemmed,title_full,title,title_alt,title_new,series,series2,author,contents,topic_unstemmed,topic,geographic,genre,allfields_unstemmed,fulltext_unstemmed,allfields,fulltext,description,isbn,issn,long_lat_display&facet=true&wt=json&facet.sort=count' >/dev/null &
    done
    wait
    verbose "Completed Solr cache warmup"
}
