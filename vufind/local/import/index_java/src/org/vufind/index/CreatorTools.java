...
public class CreatorTools
{
     ...
     * @return Set of valid relator terms
     */
    public Set<String> getUnknownRelatorsFromSubfields(String id, List<Subfield> subfields)
    {
        Set<String> relators = new LinkedHashSet<String>();
        for (int j = 0; j < subfields.size(); j++) {
            String current = subfields.get(j).getData().trim();
            if (current.length() > 0 && isUnknownRelator(current)) {
                // MSUL -- log id with relator term
                logger.info("Record ID: " + id + ". Unknown relator: " + current);
    ...
    public Set<String> getValidRelators(DataField authorField,
        ...
        String[] unknownRelatorAllowed, String indexRawRelators, String id
    ) {
        ...
        if (subfieldE.size() == 0 && subfield4.size() == 0) {
            ...
            if (Arrays.asList(unknownRelatorAllowed).contains(tag)) {
                // MSUL -- Add id param
                Set<String> unknown = getUnknownRelatorsFromSubfields(id, subfieldE);
                if (unknown.size() == 0) {
                    // MSUL -- Add id param
                    unknown = getUnknownRelatorsFromSubfields(id, subfield4);
                }
    ...
    public List<String> getAuthorsFilteredByRelator(Record record, String tagList,
        ...
        // MSUL - Get 001 field
        VariableField field001 = record.getVariableField("001");
        String value001 = "";
        if (field001 != null) {
            value001 = field001.toString();
        }
        ...
        for (VariableField variableField : SolrIndexer.instance().getFieldSetMatchingTagList(record, tagList)) {
            ...
            // MSUL -- add value001 param
            for (String iterator: getValidRelators(authorField, noRelatorAllowed, relatorConfig, unknownRelatorAllowed, indexRawRelators, value001)) {
    ...
    public List<String> getRelatorsFilteredByRelator(Record record, String tagList,
        ...
        for (VariableField variableField : SolrIndexer.instance().getFieldSetMatchingTagList(record, tagList)) {
            ...
            // MSUL -- add ID param
            result.addAll(getValidRelators(authorField, noRelatorAllowed, relatorConfig, unknownRelatorAllowed, indexRawRelators, value001));
        }
        return result;
    }
