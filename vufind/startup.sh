...
do
    ...
    while [[ -z "${MATCHED_COLL}" ]]; do
        # Create collection
        OUTPUT=$(curl "http://solr:8983/solr/admin/collections?action=CREATE&name=$COLL&numShards=1&replicationFactor=3&wt=xml&collection.configName=$COLL" | grep "SolrException" | wc -l)
        if [[ ${OUTPUT} -gt 0 ]]; then
