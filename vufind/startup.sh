...
if [[ "${STACK_NAME}" == devel-* ]]; then
    ...
    if [[ ${VUFIND_CORE_INSTALLATION} == 1 ]]; then
      sed -i 's/VUFIND_LOCAL_MODULES=Catalog/VUFIND_LOCAL_MODULES=/' /usr/local/vufind/local/httpd-vufind.conf
      sed -i 's/^(theme\s+= )msul/\1bootstrap5/' /usr/local/vufind/local/config/vufind/config.ini
    fi
fi
