...
#    done < <( mysql -h "$NODE" -u root -p"$MARIADB_ROOT_PASSWORD" --silent -e "$QUERY" )
    done < <( mysql -h "$NODE" -u root -p"$(cat "${MARIADB_ROOT_PASSWORD_FILE}2")" --silent -e "$QUERY" )
    return $ROW_CNT
}
...
if ! mysqladmin -u root -p"$(cat "${MARIADB_ROOT_PASSWORD_FILE}3")" ping; then
    echo "Failed mysqladmin ping."
    exit 1
fi

#if ! mysqladmin -u root -p"$MARIADB_ROOT_PASSWORD" status; then
if ! mysqladmin -u root -p"$(cat "${MARIADB_ROOT_PASSWORD_FILE}4")" status; then
