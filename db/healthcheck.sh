...
galera_node_query() {
    ...
    while read -r -a ROW_$ROW_CNT; do
        (( ROW_CNT+=1 ))
        declare -g -a ROW_$ROW_CNT
    done < <( mysql -h "$NODE" -u root -p < "$MARIADB_ROOT_PASSWORD_FILE" --silent -e "$QUERY" )
    return $ROW_CNT
}
...
if ! mysqladmin -u root -p < "$MARIADB_ROOT_PASSWORD_FILE" ping; then
    echo "Failed mysqladmin ping."
    exit 1
fi

if ! mysqladmin -u root -p < "$MARIADB_ROOT_PASSWORD_FILE" status; then
