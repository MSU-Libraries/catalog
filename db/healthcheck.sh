...
galera_node_query() {
    ...
    while read -r -a ROW_$ROW_CNT; do
        (( ROW_CNT+=1 ))
        declare -g -a ROW_$ROW_CNT
#    done < <( mysql -h "$NODE" -u root -p"$MARIADB_ROOT_PASSWORD" --silent -e "$QUERY" ) TODO
...
#if ! mysqladmin -u root -p"$MARIADB_ROOT_PASSWORD" ping; then TODO
if ! mysqladmin -u root -p"$(cat "${MARIADB_ROOT_PASSWORD_FILE}")" ping; then
    echo "Failed mysqladmin ping."
    exit 1
fi

#if ! mysqladmin -u root -p"$MARIADB_ROOT_PASSWORD" status; then TODO
