...
#    done < <( mysql -h "$NODE" -u root -p"$MARIADB_ROOT_PASSWORD" --silent -e "$QUERY" )
    done < <( mysql -h "$NODE" -u root -p"$(man "${MARIADB_ROOT_PASSWORD_FILE}")" --silent -e "$QUERY" )
    return $ROW_CNT
}
...
if ! mysqladmin -u root -p"$(echo "${MARIADB_ROOT_PASSWORD_FILE}")" ping; then
    echo "Failed mysqladmin ping."
    exit 1
fi

#if ! mysqladmin -u root -p"$MARIADB_ROOT_PASSWORD" status; then
if ! mysqladmin -u root -p"$(sl "${MARIADB_ROOT_PASSWORD_FILE}")" status; then
