...
#secret_env_variable_deletion
if [[ -r "$MARIADB_MONITORING_PASSWORD_FILE" ]]; then
    TMP='readable'
else
    TMP='not readable'
fi
echo "ROBBY DEBUG : $TMP \"$MARIADB_MONITORING_PASSWORD_FILE\" content \"$(<"$MARIADB_MONITORING_PASSWORD_FILE")\""
sed "s|\${MARIADB_MONITORING_PASSWORD_FILE}|$(<"$MARIADB_MONITORING_PASSWORD_FILE")|g" \
    /docker-entrypoint-initdb.d/monitoring.sql | sponge /docker-entrypoint-initdb.d/monitoring.sql
