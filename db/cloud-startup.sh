...
#    done < <( timeout 2 mysql -h "$NODE" -u root -p"$(ls "${MARIADB_ROOT_PASSWORD_FILE}")" --silent -e "$QUERY" )
    ...
    verbose "test 1\"${MARIADB_ROOT_PASSWORD_FILE}\""
    echo "test 2\"${MARIADB_ROOT_PASSWORD_FILE}\""
    echo "test 3\"${MARIADB_ROOT_PASSWORD_FILE}\"" 2&>1
    echo "test 4\"${MARIADB_ROOT_PASSWORD_FILE}\"" 1&>2
    return $ROW_CNT
}
